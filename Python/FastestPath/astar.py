...
def astar(maze, start, end):
    ...
    while len(open_list) > 0:
        ...
        """make the robot go to obstacle instead of end"""
        # if current_node != 
        
        # Found the goal -> only if robot met both obstacles
        ...
        for child in children:
            ...
            """
            Add a counter
            - if counter <2, robot has to meet the obstacle
            - such that when counter = 2, do not have to care about meeting the obstacles
            
            - add walls
            - robot has to go back to the same postion
            """
    ...
    for row in range(len(maze)):
        for col in range(len(maze[row])):
            if (row, col) == path[0]:
                ...
                print("U", end=" ")  # Up obstacle, robot will turn left
            elif maze[row][col] == "D":
                print("D", end=" ")  # Down obstacle, robot will turn right
